...
export const FinalFlyingListSection: React.FC<FinalFlyingListSectionProps> = ({
  ...
  return (
    <div className="space-y-6 font-sans text-slate-800 dark:text-slate-100 pb-12">
      ...
      <div className={`p-4 rounded-none border space-y-3 ${isDark ? 'bg-[#1E293B] border-slate-700' : 'bg-white border-slate-300'}`}>
        <div className="flex items-center justify-between">
      ...
      {filteredProposals.length === 0 ? (
        <div className={`p-12 text-center rounded-none border ${isDark ? 'bg-[#1E293B] border-slate-700' : 'bg-white border-slate-300'}`}>
            ...
            return (
              <div
                ...
                className={`p-5 rounded-none border transition-all space-y-4 ${
                  isDispatched
                    ? (isDark ? 'bg-[#1E293B] border-emerald-800/60' : 'bg-white border-emerald-300')
                    : (isDark ? 'bg-[#1E293B] border-blue-800/60' : 'bg-white border-blue-300')
                }`}
              >
                {/* Header Row */}
        ...
        <div className={`p-4 rounded-none border ${isDark ? 'bg-[#1E293B] border-slate-700' : 'bg-white border-slate-300'}`}>
      ...
      {selectedProposalForModal && (
        <div className="fixed inset-0 bg-[#1E293B]/60 backdrop-blur-xs z-50 flex items-center justify-center p-2 sm:p-4 font-sans">
          ...
          >
                ...
                <table className="w-full text-left text-xs whitespace-nowrap border-collapse border border-slate-200 dark:border-slate-700 font-light">
                  <thead className={`uppercase text-[10px] tracking-wider ${isDark ? 'bg-[#1E293B] text-slate-400' : 'bg-slate-100 text-slate-700 font-normal'}`}>
                    <tr>
                      ...
                      <th className="p-2.5 border border-slate-200 dark:border-slate-700">CTN NO</th>
                      <th className="p-2.5 border border-slate-200 dark:border-slate-700">SHIPPING MARK</th>
                      <th className="p-2.5 border border-slate-200 dark:border-slate-700">PRODUCT NAME</th>
                      <th className="p-2.5 border border-slate-200 dark:border-slate-700">QTY / N.WT</th>
                      <th className="p-2.5 border border-slate-200 dark:border-slate-700">G.WEIGHT</th>
                      <th className="p-2.5 border border-slate-200 dark:border-slate-700">CBM</th>
                      <th className="p-2.5 border border-slate-200 dark:border-slate-700">TRACKING NO</th>
                      <th className="p-2.5 border border-slate-200 dark:border-slate-700">PROOF</th>
                      <th className="p-2.5 border border-slate-200 dark:border-slate-700">DESTINATION</th>
                      <th className="p-2.5 border border-slate-200 dark:border-slate-700">STATUS</th>
                    </tr>
                  </thead>
                  <tbody className={isDark ? 'text-slate-200' : 'text-slate-800'}>
                    {modalProposalCartons.length === 0 ? (
                      <tr>
                        <td colSpan={12} className="p-8 text-center text-slate-400 border border-slate-200 dark:border-slate-700 font-light">
                      ...
                      modalProposalCartons.map((c, idx) => (
                        <tr key={c.id} className="hover:bg-slate-50/50 dark:hover:bg-[#1E293B] transition-colors">
                          ...
                          <td className="p-2.5 font-normal border border-slate-200 dark:border-slate-700 max-w-[200px] truncate">{c.product_name_en}</td>
                          <td className="p-2.5 font-mono text-center border border-slate-200 dark:border-slate-700">{c.quantity || 1} pcs | {c.net_weight || 0} kg</td>
                          <td className="p-2.5 font-mono text-emerald-600 dark:text-emerald-400 font-normal border border-slate-200 dark:border-slate-700 text-center">{c.gross_weight} kg</td>
                          <td className="p-2.5 font-mono text-purple-600 dark:text-purple-400 font-normal border border-slate-200 dark:border-slate-700 text-center">{c.cbm} CBM</td>
                          <td className="p-2.5 font-mono text-slate-500 border border-slate-200 dark:border-slate-700">{c.tracking_number}</td>
                          ...
                          <td className="p-2.5 font-normal text-emerald-600 dark:text-emerald-400 border border-slate-200 dark:border-slate-700">{c.destination_warehouse_name || 'ঢাকা সেন্ট্রাল (BD)'}</td>
            ...
            {/* Modal Footer */}
            <div className="border-t pt-3 border-slate-200 dark:border-slate-700 flex justify-end">
              <button
                onClick={() => setSelectedProposalForModal(null)}
