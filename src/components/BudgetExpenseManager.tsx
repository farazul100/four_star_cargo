...
export const BudgetExpenseManager: React.FC<BudgetExpenseManagerProps> = ({
  ...
  const [expenses, setExpenses] = useState<ExpenseItem[]>(dbData.expenses || []);
  ...
  // Mock Income Constant (Calculated from cargo weight/cbm)
  ...
  const netMarginPercent = ((netProfitOrLoss / totalCargoIncome) * 100).toFixed(1);
  const expenseRatio = ((totalFilteredExpense / totalCargoIncome) * 100).toFixed(1);
  ...
  const getCategoryBadgeClass = (cat: ExpenseItem['category']) => {
    switch (cat) {
      case 'shipping':
  ...
  return (
    <div className="font-sans">
      ...
      <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-3.5 ${isDark ? 'border-[#2C2C2E]' : 'border-slate-200'}`}>
        <div>
          <h1 className={`text-lg md:text-xl font-semibold flex items-center space-x-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <div className="flex items-center space-x-2 px-3 py-1 rounded-full bg-slate-100 border border-slate-200 text-slate-700 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300 text-xs font-medium">
            <span className="w-1.5 h-1.5 rounded-full bg-[#00897B] animate-pulse" />
            <span>{isBn ? 'অ্যাকাউন্টস ম্যানেজার প্যানেল থেকে সিঙ্ককৃত' : 'Live Synced from Accountant Panel'}</span>
          </div>

          <button
            onClick={() => setShowAddModal(true)}
            className="px-3.5 py-1.5 rounded-xl text-xs font-semibold bg-[#00897B] hover:bg-[#00796B] text-white hover:scale-102 active:scale-98 transition-all duration-200 flex items-center space-x-1.5 cursor-pointer shadow-xs"
          >
            <PlusCircle className="w-3.5 h-3.5" />
            <span>{isBn ? 'নতুন খরচ যোগ করুন' : 'Record New Expense'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
        ...
        <div className={`p-4 rounded-xl border transition-all ${
          ...
          <div className="flex items-center justify-between">
            <span className={`text-xs font-medium ${isDark ? 'text-gray-400' : 'text-slate-600'}`}>{isBn ? 'মোট কার্গো বুকিং আয়' : 'Total Cargo Revenue'}</span>
          ...
          <p className={`text-[11px] mt-0.5 ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>{isBn ? 'বিতরণকৃত ও বুকড কার্টুন রাজস্ব' : 'Delivered & booked cargo fees'}</p>
        </div>
        ...
        <div className={`p-4 rounded-xl border transition-all ${
          ...
          <div className="flex items-center justify-between">
            <span className={`text-xs font-medium ${isDark ? 'text-gray-400' : 'text-slate-600'}`}>{isBn ? 'ফিল্টারকৃত মোট ব্যয়' : 'Total Filtered Expense'}</span>
        ...
        <div className={`p-4 rounded-xl border transition-all ${
          ...
          <div className="flex items-center justify-between">
            <span className={`text-xs font-medium ${isDark ? 'text-gray-400' : 'text-slate-600'}`}>{isBn ? 'আয়-ব্যয়ের নিট পার্থক্য' : 'Net Profit / Difference'}</span>
          ...
          <p className={`text-[11px] mt-0.5 font-medium ${netProfitOrLoss >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-700'}`}>
        ...
        <div className={`p-4 rounded-xl border ${
          ...
          <div className="flex items-center justify-between">
            <span className={`text-xs font-medium ${isDark ? 'text-gray-400' : 'text-slate-600'}`}>{isBn ? 'ব্যয়ের শতাংশ (Ratio)' : 'Expense-to-Income Ratio'}</span>
            <div className="text-purple-600 dark:text-purple-400">
              <PieChart className="w-4 h-4" />
            </div>
          </div>
          <p className="text-xl font-bold font-mono mt-2 text-purple-700 dark:text-purple-400">{expenseRatio}%</p>
          <p className={`text-[11px] mt-0.5 ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>{isBn ? 'আয়ের বিপরীতে খরচ' : 'Expense % of total income'}</p>
        </div>
      </div>

      {/* 3. Smart Date Filter Bar */}
      <div className={`p-3.5 rounded-xl border flex flex-wrap items-center justify-between gap-2.5 text-xs ${
        isDark ? 'bg-[#1C1C1E] border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-800 shadow-sm'
      }`}>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative min-w-[220px]">
            ...
            <input
              ...
              className={`w-full border rounded-lg py-1.5 pl-8 pr-3 text-xs outline-none font-medium ${
            ...
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className="absolute right-2.5 top-2.5 text-gray-400 hover:text-gray-900 dark:hover:text-white border-0 bg-transparent cursor-pointer">
          ...
          {/* Category Selector */}
          <div className={`flex items-center space-x-2 border rounded-lg px-2.5 py-1.5 ${isDark ? 'bg-[#121214] border-slate-700 text-white' : 'bg-slate-50 border-slate-200 text-slate-900'}`}>
            <Filter className="w-3.5 h-3.5 text-slate-500" />
            <select
              value={categoryFilter}
              onChange={(e) => setCategoryFilter(e.target.value)}
              className="bg-transparent outline-none cursor-pointer text-xs font-medium dark:bg-[#121214] dark:text-white"
            >
              <option value="all" className="bg-white text-gray-900 dark:bg-[#1C1C1E] dark:text-white">{isBn ? 'সব খাতের খরচ (All Categories)' : 'All Expense Categories'}</option>
              <option value="shipping" className="bg-white text-gray-900 dark:bg-[#1C1C1E] dark:text-white">এয়ার ফ্রাইট ও শিপিং চার্জ</option>
              <option value="warehouse_rent" className="bg-white text-gray-900 dark:bg-[#1C1C1E] dark:text-white">ওয়্যারহাউজ ভাড়া ও লিজ</option>
              <option value="salary" className="bg-white text-gray-900 dark:bg-[#1C1C1E] dark:text-white">স্টাফ বেতন ও ওভারটাইম</option>
              <option value="customs" className="bg-white text-gray-900 dark:bg-[#1C1C1E] dark:text-white">কাস্টমস শুল্ক ও ট্যাক্স</option>
              <option value="packing_transport" className="bg-white text-gray-900 dark:bg-[#1C1C1E] dark:text-white">প্যাকিং ও ট্রান্সপোর্ট</option>
              <option value="utilities" className="bg-white text-gray-900 dark:bg-[#1C1C1E] dark:text-white">ইউটিলিটি ও বিল</option>
            </select>
          </div>
          ...
          <div className={`flex items-center space-x-2 border rounded-lg px-2.5 py-1.5 ${isDark ? 'bg-[#121214] border-slate-700 text-white' : 'bg-slate-50 border-slate-200 text-slate-900'}`}>
            ...
            <select
              ...
              className="bg-transparent outline-none cursor-pointer text-xs font-medium text-slate-800 dark:text-emerald-400"
            >
          ...
          {dateFilterType === 'date_range' && (
            <div className="flex items-center space-x-2">
              <div className={`flex items-center space-x-2 border rounded-lg px-2.5 py-1.5 ${isDark ? 'bg-[#121214] border-slate-700 text-white' : 'bg-slate-50 border-slate-200 text-slate-900'}`}>
                <span className="text-[11px] font-medium">{isBn ? 'হতে:' : 'From:'}</span>
                <input
                  type="date"
                  value={startDate}
              ...
              <div className={`flex items-center space-x-2 border rounded-lg px-2.5 py-1.5 ${isDark ? 'bg-[#121214] border-slate-700 text-white' : 'bg-slate-50 border-slate-200 text-slate-900'}`}>
                <span className="text-[11px] font-medium">{isBn ? 'পর্যন্ত:' : 'To:'}</span>
                <input
                  type="date"
                  value={endDate}
          ...
          {dateFilterType !== 'all' && (
            <button
              ...
              className={`px-2.5 py-1.5 rounded-lg border text-xs font-medium flex items-center space-x-1 cursor-pointer outline-none transition-all ${
                isDark
                  ? 'bg-[#121214] hover:bg-slate-700 border-slate-700 text-rose-400'
      ...
      {/* 4. Category Wise Breakdown (কি বাবদ কি খরচ হচ্ছে) */}
      <div className={`p-5 rounded-xl border space-y-3.5 shadow-sm ${
            ...
            return (
              <div
                ...
                className={`p-4 rounded-xl border flex flex-col justify-between transition-all ${
                  isDark ? 'bg-[#121214] border-slate-700' : 'bg-white border-slate-200 hover:border-slate-300 shadow-sm'
                }`}
              >
                <div>
                  <div className="flex items-center justify-between gap-2">
                    <span className={`text-xs font-semibold ${isDark ? 'text-gray-200' : 'text-slate-900'}`}>
                      {getCategoryLabel(catKey as ExpenseItem['category'])}
                    </span>
                    <span className={`text-xs font-mono font-semibold ${isDark ? 'text-gray-400' : 'text-slate-600'}`}>{pct}%</span>
                  </div>
                  <p className={`text-xl font-bold font-mono mt-2.5 ${isDark ? 'text-white' : 'text-slate-900'}`}>৳{catSum.toLocaleString()}</p>
                </div>

                {/* Progress bar */}
                <div className={`w-full h-2 rounded-full mt-3.5 overflow-hidden ${isDark ? 'bg-gray-800' : 'bg-slate-100 border border-slate-200'}`}>
                  <div className="h-full bg-[#00897B] rounded-full transition-all duration-300" style={{ width: `${pct}%` }} />
                </div>
              </div>
            );
          })}
      ...
      <div className={`p-5 rounded-xl border space-y-3.5 shadow-sm ${
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-700 pb-2.5">
          <div className="flex items-center space-x-2">
            <BarChart3 className="w-4 h-4 text-[#00897B]" />
            <h2 className="text-sm font-semibold">{isBn ? 'আর্থিক বিবরণী ও হিসেব রিপোর্ট (Financial Statements)' : 'Periodical Financial Statements'}</h2>
          </div>

          <div className="flex items-center space-x-2">
            <div className={`flex rounded-lg p-0.5 border ${isDark ? 'bg-[#121214] border-slate-700' : 'bg-slate-100 border-slate-200'}`}>
              <button
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-md text-xs font-normal transition-all cursor-pointer ${
                  reportTab === 'monthly'
            ...
            <button
              ...
              className={`p-1.5 rounded-lg border text-xs font-normal flex items-center space-x-1.5 transition-all cursor-pointer ${
                isDark ? 'bg-[#121214] border-[#2C2C2E] text-white hover:bg-[#222224]' : 'bg-white border-slate-200/60 text-slate-700 hover:bg-slate-50 shadow-2xs'
              }`}
              title={isBn ? 'রিপোর্ট প্রিন্ট বা ডাউনলোড করুন' : 'Print / Export Report'}
            >
              <Printer className="w-3.5 h-3.5 opacity-80" />
              <span className="hidden sm:inline">{isBn ? 'প্রিন্ট রিপোর্ট' : 'Print'}</span>
            </button>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className={`w-full text-left text-xs ${isDark ? 'text-white' : 'text-slate-700'}`}>
            <thead className={`uppercase text-[10px] tracking-wider border-b ${
              isDark ? 'bg-[#121214] text-[#9E9E9E] border-[#2C2C2E]' : 'bg-slate-50/70 text-slate-500 font-medium border-slate-200/60'
            }`}>
              <tr>
                <th className="p-2.5">সময়সীমা / তারিখ</th>
                <th className="p-2.5">মোট কার্গো আয় (৳)</th>
                <th className="p-2.5">মোট পরিচালন ব্যয় (৳)</th>
                <th className="p-2.5">আয়-ব্যয়ের নিট পার্থক্য (৳)</th>
                <th className="p-2.5">ব্যয়ের অনুপাত (%)</th>
                <th className="p-2.5 text-right">অবস্থা (Profit Status)</th>
              </tr>
            </thead>
            <tbody className={`divide-y ${isDark ? 'divide-[#2C2C2E]' : 'divide-slate-100'}`}>
              {reportTab === 'daily' && (
                <>
                  <tr className={isDark ? 'hover:bg-[#222224]' : 'hover:bg-slate-50/60'}>
                    <td className="p-2.5 font-medium">আজ (১৫ আগস্ট ২০২৬)</td>
                    <td className="p-2.5 font-mono text-emerald-600 dark:text-emerald-400">৳১৪৫,০০০</td>
                    <td className="p-2.5 font-mono text-rose-500 dark:text-rose-400">৳৬৫,০০০</td>
                    <td className="p-2.5 font-mono font-medium text-teal-600 dark:text-teal-400">+৳৮০,০০০</td>
                    <td className="p-2.5 font-mono">44.8%</td>
                    <td className="p-2.5 text-right font-medium text-emerald-600 dark:text-emerald-400">উদ্বৃত্ত (Profit)</td>
                  </tr>
                  <tr className={isDark ? 'hover:bg-[#222224]' : 'hover:bg-slate-50/60'}>
                    <td className="p-2.5 font-medium">গতকাল (১৪ আগস্ট ২০২৬)</td>
                    <td className="p-2.5 font-mono text-emerald-600 dark:text-emerald-400">৳৯৫০,০০০</td>
                    <td className="p-2.5 font-mono text-rose-500 dark:text-rose-400">৳৮৫০,০০০</td>
                    <td className="p-2.5 font-mono font-medium text-teal-600 dark:text-teal-400">+৳১০০,০০০</td>
                    <td className="p-2.5 font-mono">89.4%</td>
                    <td className="p-2.5 text-right font-medium text-emerald-600 dark:text-emerald-400">উদ্বৃত্ত (Profit)</td>
                  </tr>
                </>
              )}
              ...
              {reportTab === 'monthly' && (
                <>
                  <tr className={isDark ? 'hover:bg-[#222224]' : 'hover:bg-slate-50/60'}>
                    <td className="p-2.5 font-medium">আগস্ট ২০২৬ (বর্তমান মাস)</td>
                    <td className="p-2.5 font-mono text-emerald-600 dark:text-emerald-400">৳২,৮৫০,০০০</td>
                    <td className="p-2.5 font-mono text-rose-500 dark:text-rose-400">৳১,৬৪৫,০০০</td>
                    <td className="p-2.5 font-mono font-medium text-teal-600 dark:text-teal-400">+৳১,২০৫,০০০</td>
                    <td className="p-2.5 font-mono">57.7%</td>
                    <td className="p-2.5 text-right font-medium text-emerald-600 dark:text-emerald-400">উদ্বৃত্ত (Profit 42.3%)</td>
                  </tr>
                  <tr className={isDark ? 'hover:bg-[#222224]' : 'hover:bg-slate-50/60'}>
                    <td className="p-2.5 font-medium">জুলাই ২০২৬</td>
                    <td className="p-2.5 font-mono text-emerald-600 dark:text-emerald-400">৳৩,১০০,০০০</td>
                    <td className="p-2.5 font-mono text-rose-500 dark:text-rose-400">৳১,৭২০,০০০</td>
                    <td className="p-2.5 font-mono font-medium text-teal-600 dark:text-teal-400">+৳১,৩৮০,০০০</td>
                    <td className="p-2.5 font-mono">55.4%</td>
                    <td className="p-2.5 text-right font-medium text-emerald-600 dark:text-emerald-400">উদ্বৃত্ত (Profit 44.6%)</td>
                  </tr>
                </>
              )}
              ...
              {reportTab === 'yearly' && (
                <>
                  <tr className={isDark ? 'hover:bg-[#222224]' : 'hover:bg-slate-50/60'}>
                    <td className="p-2.5 font-medium">২০২৬ অর্থবছর (YTD 2026)</td>
                    <td className="p-2.5 font-mono text-emerald-600 dark:text-emerald-400">৳২২,৪০০,০০০</td>
                    <td className="p-2.5 font-mono text-rose-500 dark:text-rose-400">৳১২,৮০০,০০০</td>
                    <td className="p-2.5 font-mono font-medium text-teal-600 dark:text-teal-400">+৳৯,৬০০,০০০</td>
                    <td className="p-2.5 font-mono">57.1%</td>
                    <td className="p-2.5 text-right font-medium text-emerald-600 dark:text-emerald-400">বার্ষিক নিট লাভ (42.9%)</td>
                  </tr>
                  <tr className={isDark ? 'hover:bg-[#222224]' : 'hover:bg-slate-50/60'}>
                    <td className="p-2.5 font-medium">২০২৫ অর্থবছর (Full Year 2025)</td>
                    <td className="p-2.5 font-mono text-emerald-600 dark:text-emerald-400">৳১৮,৯০০,০০০</td>
                    <td className="p-2.5 font-mono text-rose-500 dark:text-rose-400">৳১১,১৫০,০০০</td>
                    <td className="p-2.5 font-mono font-medium text-teal-600 dark:text-teal-400">+৳৭,৭৫০,০০০</td>
                    <td className="p-2.5 font-mono">58.9%</td>
                    <td className="p-2.5 text-right font-medium text-emerald-600 dark:text-emerald-400">বার্ষিক নিট লাভ (41.1%)</td>
                  </tr>
                </>
              )}
            </tbody>
      ...
      <div className={`border rounded-xl overflow-hidden shadow-2xs ${
        isDark ? 'bg-[#1C1C1E] border-[#2C2C2E]/80 text-white' : 'bg-white border-slate-200/60 text-slate-700'
      }`}>
        <div className="p-3.5 border-b border-slate-200/60 dark:border-[#2C2C2E] flex items-center justify-between">
          <h2 className="text-sm font-medium flex items-center space-x-2">
            <FileSpreadsheet className="w-4 h-4 text-[#00897B] opacity-80" />
          ...
          <span className={`text-xs ${isDark ? 'text-gray-400' : 'text-slate-400'}`}>
            {filteredExpenses.length} {isBn ? 'টি খরচ ফিল্টার করা হয়েছে' : 'entries'}
          </span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className={`w-full text-left text-xs ${isDark ? 'text-white' : 'text-slate-900'}`}>
            <thead className={`uppercase text-[10px] tracking-wider border-b ${
              isDark ? 'bg-[#121214] text-[#9E9E9E] border-[#2C2C2E]' : 'bg-slate-100 text-slate-900 border-slate-300 font-bold'
            }`}>
              <tr>
            ...
            <tbody className={`divide-y ${isDark ? 'divide-[#2C2C2E]' : 'divide-slate-200'}`}>
              {filteredExpenses.length === 0 ? (
                <tr>
                  <td colSpan={6} className={`p-6 text-center text-xs ${isDark ? 'text-gray-400' : 'text-slate-600'}`}>
                    {isBn ? 'কোনো খরচের ভাউচার পাওয়া যায়নি' : 'No expenses found matching your filter'}
                  </td>
                </tr>
              ) : (
                filteredExpenses.map((exp) => (
                  <tr key={exp.id} className={isDark ? 'hover:bg-[#222224] transition-colors' : 'hover:bg-slate-50 transition-colors'}>
                    <td className={`p-3 font-mono font-medium ${isDark ? 'text-gray-300' : 'text-slate-900'}`}>{exp.date}</td>
                    <td className={`p-3 font-mono font-bold ${isDark ? 'text-teal-400' : 'text-teal-700'}`}>{exp.voucher_no}</td>
                    <td className="p-3">
                      <span className={`font-semibold text-xs ${isDark ? 'text-gray-200' : 'text-slate-900'}`}>
                        {getCategoryLabel(exp.category)}
                      </span>
                    </td>
                    <td className="p-3">
                      <p className={`font-semibold text-xs ${isDark ? 'text-white' : 'text-slate-900'}`}>{exp.title}</p>
                      {exp.notes && <p className={`text-[11px] mt-0.5 ${isDark ? 'text-gray-400' : 'text-slate-600'}`}>{exp.notes}</p>}
                    </td>
                    <td className={`p-3 font-medium uppercase text-[11px] ${isDark ? 'text-gray-300' : 'text-slate-900'}`}>
                      {exp.payment_method.replace('_', ' ')}
                    </td>
                    <td className={`p-3 text-right font-mono font-bold text-sm ${isDark ? 'text-rose-400' : 'text-rose-700'}`}>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-backdrop-blur-fade">
          <div className={`w-full max-w-lg border rounded-2xl p-6 space-y-5 shadow-2xl relative animate-modal-pop-bounce ${
            ...
            <form onSubmit={handleCreateExpense} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full border rounded-xl p-2.5 text-xs outline-none ${
                    isDark ? 'bg-[#121214] border-[#2C2C2E] text-white' : 'bg-slate-50 border-slate-300 text-slate-900'
                  }`}
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    <option value="shipping" className={isDark ? 'bg-[#1C1C1E] text-white' : 'bg-white text-slate-900'}>এয়ার ফ্রাইট ও শিপিং চার্জ</option>
                    <option value="warehouse_rent" className={isDark ? 'bg-[#1C1C1E] text-white' : 'bg-white text-slate-900'}>ওয়্যারহাউজ ভাড়া ও লিজ</option>
                    <option value="salary" className={isDark ? 'bg-[#1C1C1E] text-white' : 'bg-white text-slate-900'}>স্টাফ বেতন ও ওভারটাইম</option>
                    <option value="customs" className={isDark ? 'bg-[#1C1C1E] text-white' : 'bg-white text-slate-900'}>কাস্টমস শুল্ক ও ট্যাক্স</option>
                    <option value="packing_transport" className={isDark ? 'bg-[#1C1C1E] text-white' : 'bg-white text-slate-900'}>প্যাকিং ও ট্রান্সপোর্ট</option>
                    <option value="utilities" className={isDark ? 'bg-[#1C1C1E] text-white' : 'bg-white text-slate-900'}>ইউটিলিটি ও বিল</option>
                    <option value="other" className={isDark ? 'bg-[#1C1C1E] text-white' : 'bg-white text-slate-900'}>অন্যান্য প্রশাসনিক খরচ</option>
                  </select>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  >
                    <option value="bank_transfer" className={isDark ? 'bg-[#1C1C1E] text-white' : 'bg-white text-slate-900'}>Bank Transfer (ব্যাংক ট্রান্সফার)</option>
                    <option value="cash" className={isDark ? 'bg-[#1C1C1E] text-white' : 'bg-white text-slate-900'}>Cash (ক্যাশ পেমেন্ট)</option>
                    <option value="mobile_banking" className={isDark ? 'bg-[#1C1C1E] text-white' : 'bg-white text-slate-900'}>Mobile Banking (বিকাশ/নগদ)</option>
                  </select>
                </div>
              </div>
              ...
              <div className="flex justify-end space-x-3 pt-3 border-t">
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all cursor-pointer ${
                    isDark ? 'bg-[#121214] border-[#2C2C2E] text-gray-400 hover:text-white' : 'bg-slate-100 border-slate-200 text-slate-700 hover:bg-slate-200'
                  }`}
                >
                  {isBn ? 'বাতিল' : 'Cancel'}
                </button>
                ...
                <button
                  type="submit"
                  className="px-5 py-2 rounded-xl text-xs font-bold bg-[#00897B] text-white hover:bg-[#00796B] transition-all cursor-pointer shadow-md"
                >
      ...
      {/* 8. OFFICIAL EXECUTIVE PRINTABLE REPORT (Hides all UI, prints official letterhead report) */}
      <div className="hidden print:block printable-document text-slate-900 bg-white p-6 font-sans">
        ...
        <div className="border-b-2 border-slate-900 pb-4 mb-6 flex justify-between items-start">
          ...
          <div className="text-right border-l pl-4 border-slate-300">
            <span className="inline-block px-2.5 py-1 bg-slate-900 text-white text-[10px] font-bold tracking-widest uppercase rounded">
              Official Document
            </span>
        ...
        {/* Document Title */}
        <div className="text-center bg-slate-100 py-3 rounded-lg border border-slate-300 mb-6">
