...
export const WarehouseSetupManager: React.FC<WarehouseSetupManagerProps> = ({
  ...
  const handleCreateWarehouse = (e: React.FormEvent) => {
    ...
    addToast(
      'success',
      isBn ? 'নতুন ওয়্যারহাউজ তৈরি সফল!' : 'Warehouse Created Successfully!',
      isBn ? `${newWh.name} সফলভাবে সিস্টেমে যুক্ত হয়েছে।` : `${newWh.name} is now active in system.`
    );
  };

  // ACTION: ADD INCHARGE USER TO SPECIFIC WAREHOUSE
  const handleAddInchargeToWarehouse = (e: React.FormEvent) => {
    e.preventDefault();
    if (!targetWhForIncharge || !inchargeName.trim() || !inchargeEmail.trim()) {
      addToast('error', isBn ? 'তথ্য অসম্পূর্ণ' : 'Incomplete Form', isBn ? 'ইনচার্জ কর্মকর্তার নাম ও ইমেইল বাধ্যতামূলক' : 'Name & Email are required');
      return;
    }

    const newInchargeId = `usr-inc-${Date.now().toString().slice(-4)}`;
    const newInchargeStaff: WarehouseInchargeStaff = {
      id: newInchargeId,
      name: inchargeName,
      email: inchargeEmail,
      phone: inchargePhone || '+880 1700-112233',
      role: 'warehouse_incharge',
      status: 'active',
      created_at: new Date().toISOString(),
    };

    // Update Warehouse's incharge_staff array
    const updatedWhs = warehouses.map((w) => {
      if (w.id === targetWhForIncharge.id) {
        const existingStaff = w.incharge_staff || [];
        return {
          ...w,
          incharge_staff: [...existingStaff, newInchargeStaff],
        };
      }
      return w;
    });

    // Also add to persistent Users array with warehouse_id mapping
    const newUserAccount: User = {
      id: newInchargeId,
      name: `${inchargeName} (${targetWhForIncharge.code} Incharge)`,
      email: inchargeEmail,
      role: 'warehouse_incharge',
    ...
    const updatedUsers = [newUserAccount, ...users];
    setUsers(updatedUsers);
    saveHostingerDbData(DB_KEYS.USERS, updatedUsers);

    syncWarehouses(
      updatedWhs,
      `Super Admin created Incharge Account (${inchargeName}) bound to Warehouse ${targetWhForIncharge.name}`
    );
    ...
    <div className="space-y-5 font-sans">
      ...
      <div className={`p-4 rounded-xl border flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
        ...
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 rounded-xl bg-teal-50 border border-teal-200 text-[#00897B] dark:bg-teal-950/60 dark:border-teal-800 dark:text-teal-400 flex items-center justify-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredWarehouses.length === 0 ? (
          <div className={`col-span-full p-10 rounded-xl border text-center text-xs ${
            ...
            <p className={`font-medium ${isDark ? 'text-slate-200' : 'text-slate-900'}`}>{isBn ? 'কোনো ওয়্যারহাউজ পাওয়া যায়নি' : 'No warehouses found'}</p>
            <p className="mt-1 font-normal">{isBn ? 'নতুন ওয়্যারহাউজ যুক্ত করতে উপরে "+ নতুন ওয়্যারহাউজ যোগ করুন" বাটনে চাপ দিন।' : 'Click Create New Warehouse button to add.'}</p>
          </div>
        ) : (
            ...
            return (
              ...
              >
                <div>
                  ...
                  <div className={`flex items-start justify-between gap-2 border-b pb-3.5 ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <div className="flex items-center space-x-2">
                          <h3 className={`font-semibold text-sm leading-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>{wh.name}</h3>
                        </div>
                        <div className="flex items-center space-x-2 mt-1">
                          ...
                          <span className={`text-[11px] font-normal ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{wh.country}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className={`px-2.5 py-0.5 rounded-full text-[10px] font-medium border transition-all cursor-pointer ${
                        wh.status === 'active'
                          ? isDark
                  ...
                  <div className="space-y-2 mt-3 text-xs">
                    <div className="flex items-center justify-between text-slate-600 dark:text-slate-400">
                      <span className={`flex items-center space-x-1.5 font-normal ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                    ...
                    {wh.address && (
                      <div className={`flex items-start space-x-1.5 text-xs font-normal ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                    ...
                    {wh.phone && (
                      <div className={`flex items-center space-x-1.5 text-xs font-normal ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                    ...
                    <div className="flex items-center justify-between pt-1">
                      <span className={`text-xs font-normal ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{isBn ? 'বর্তমানে মোট স্টকড কার্টুন:' : 'Current Stocked Cartons:'}</span>
                      <span className={`font-mono font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>{totalStockCount} {isBn ? 'টি কার্টুন' : 'cartons'}</span>
                    </div>
                  </div>
                  ...
                  <div className={`mt-4 pt-3 border-t space-y-2.5 ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>
                    <div className="flex items-center justify-between">
                      <span className={`text-xs font-medium flex items-center space-x-1.5 ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
                        <Users className="w-3.5 h-3.5 text-[#00897B]" />
                        <span>{isBn ? 'ইনচার্জ কর্মকর্তাবৃন্দ (Incharge Roster)' : 'Incharge Officers Roster'}</span>
                      </span>
                      <span className={`text-[10px] font-mono ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{staffList.length} {isBn ? 'জন কর্মকর্তা' : 'staff'}</span>
                    </div>

                    {/* Staff Accounts List */}
                    <div className="space-y-1.5">
                      {staffList.length === 0 ? (
                        <div className={`p-2.5 rounded-xl border text-center text-xs font-normal ${
                          isDark ? 'bg-[#121214] border-slate-800 text-slate-400' : 'bg-slate-50 border-slate-200 text-slate-600'
                        }`}>
                        ...
                        staffList.map((stf) => (
                          ...
                          >
                            <div className="min-w-0 pr-2">
                              <p className={`font-medium text-xs truncate leading-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>{stf.name}</p>
                              <p className={`text-[10px] font-mono truncate ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{stf.email}</p>
                            </div>
                ...
                <button
                  ...
                  className={`w-full py-2 rounded-xl text-xs font-normal border border-dashed transition-all cursor-pointer flex items-center justify-center space-x-1.5 ${
                    isDark
              ...
              <div className="grid grid-cols-2 gap-3.5">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full border rounded-xl py-2.5 px-3.5 outline-none font-mono uppercase transition-all ${
                      isDark
                        ? 'bg-[#121214] border-slate-700 text-white focus:border-teal-500'
                        : 'bg-slate-50/70 border-slate-200 text-slate-900 focus:bg-white focus:border-[#00897B] focus:ring-2 focus:ring-[#00897B]/15'
                    }`}
      ...
      {targetWhForIncharge && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-backdrop-blur-fade">
          <div className={`w-full max-w-lg rounded-2xl border p-6 space-y-4 shadow-2xl animate-modal-pop-bounce ${
            isDark ? 'bg-[#1C1C1E] border-slate-800 text-white' : 'bg-white border-slate-200 text-slate-900'
          }`}>
            {/* Modal Header */}
            <div className={`flex items-center justify-between border-b pb-4 ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#00897B] font-medium mt-0.5">
            ...
            {/* Modal Form */}
            <form onSubmit={handleAddInchargeToWarehouse} className="space-y-3.5 text-xs">
              <div>
                <label className={`block text-[11px] font-medium mb-1 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
              ...
              <div className={`p-3.5 rounded-xl border text-xs font-normal ${
                ...
                <p className={`mt-1 text-[11px] leading-relaxed ${isDark ? 'text-teal-300/90' : 'text-teal-800'}`}>
                  {isBn
